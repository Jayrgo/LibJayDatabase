...
local function Restore(self)
    ...
    if type(_G[self.variableName]) == "table" then
        ...
        self.lastChangeTime = _G[self.variableName].lastChangeTime or GetServerTime()
...
---@param self table
---@param event string
local function TriggerEventOnNamepsaces(self, event, ...)
    if self.namespaces then
        for k, v in pairs(self.namespaces) do
            local callbacks = v.callbacks
            --@debug@
            callbacks:xTriggerEvent(event, v, ...)
            callbacks:xTriggerEvent("OnChanged", v)
            --@end-debug@
            --[===[@non-debug@
            callbacks:TriggerEvent(event, self, ...)
            callbacks:TriggerEvent("OnChanged", self)
...
function DatabaseMixin:SetProfile(profileKey)
    ...
    TriggerEventOnNamepsaces(self, "OnProfileChanged", profileKey)
end
...
function DatabaseMixin:ResetProfile(profileKey)
    ...
    TriggerEventOnNamepsaces(self, "OnProfileReset", profileKey)
end
...
function DatabaseMixin:DeleteProfile(profileKey)
    ...
    TriggerEventOnNamepsaces(self, "OnProfileDeleted", profileKey)
end
...
function DatabaseMixin:CopyProfile(from)
    ...
    TriggerEventOnNamepsaces(self, "OnProfileCopied", from)
end
...
function DatabaseMixin:GetGlobalDefault(...) get(self.defaults_globals, coercePath(...)) end
...
function DatabaseMixin:IsRestored()
    return self.restored
end

local NamespaceMixin = {}

function NamespaceMixin:OnLoad()
    self.OnLoad = nil
    self.callbacks = self.callbacks or LJCallback:New(self)
end
...
---@param value boolean | number | string | nil
---@param path any
function NamespaceMixin:SetDefault(value, path, ...)
    checkValue(value, "Namespace:SetDefault(value, path[, ...])")

    local parent = self.parent

    set(parent.defaults, value, "namespaces", self.key, coercePath(path, ...))

    if parent.restored then copyDefaults(parent.profiles[parent:GetProfile()], parent.defaults) end
end

---@return any defult
function NamespaceMixin:GetDefault(...) return get(self.parent.defaults, "namespaces", self.key, coercePath(...)) end

---@param value boolean | number | string | nil
---@param path string
function NamespaceMixin:Set(value, path, ...)
    checkValue(value, "Namespace:Set(value, path[, ...]")

    if set(self.parent.db, value, "namespaces", self.key, coercePath(path, ...)) then
        setLastChange(self.parent)
        TriggerEvent(self, "OnValueChanged", value, coercePath(path, ...))
    end
end
...
---@param value any
---@param path any
function NamespaceMixin:SetWithoutResponse(value, path, ...)
    checkValue(value, "NamespaceMixin:SetWithoutResponse(value, path[, ...]")
    local parent = self.parent
    if set(parent.db, value, "namespaces", self.key, coercePath(path, ...)) then setLastChange(parent) end
end

---@return boolean | number | string | nil value
function NamespaceMixin:Get(...)
    if self.parent.restored then
        local value = get(self.parent.db, "namespaces", self.key, coercePath(...))
        if type(value) ~= "nil" then return value end
    end
    return get(self.parent.defaults, "namespaces", self.key, coercePath(...))
end

local LJMixin = LibStub("LibJayMixin")
---@param key string
function DatabaseMixin:NewNamespace(key)
    if type(key) ~= "string" then
        error(format("Usage: %s:NewNamespace(key): 'key' - string expected got %s.", MAJOR, type(key), 2))
    end

    if self.namespaces and self.namespaces[key] then
        error(format("Usage: %s:NewNamespace(key): 'key' Namespace '%s' already exists.", MAJOR, key), 2)
    end

    local namespace = LJMixin:Mixin({key = key, parent = self}, NamespaceMixin)
